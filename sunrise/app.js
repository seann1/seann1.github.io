			...
			geocoder.geocode( { 'address': address}, function(results, status) {
  				if (status == google.maps.GeocoderStatus.OK)
  				{
	   				...
	   				while (counterStartDate.unix() <= counterEndDate.unix()) {
	 					...
	 					var sunriseurl = "https://api.sunrise-sunset.org/json?lat=" + latitude + "&lng=" + longitude + "&date=" + counterStartDate.format("YYYY-MM-DD")+"&callback=window.sunriseCall";
			   			var response = $.ajax({
							    		url: sunriseurl,
							    		dataType: "jsonp"
						...
						window.sunriseCall = function(data) {
					    	console.log(data);
							...
							$("#sunrises").append("<tr class='day day" + 
							i + "'><td class='date date" + i + 
							"'>" + "</td><td>" + 
							toTimeZone(data.results.sunrise) + 
							"</td><td>" + toTimeZone(data.results.sunset) + 
							"</td><td>" + data.results.day_length + 
							"</td><td><button class='btn btn-default' data-toggle='modal' data-target='#modal" + i + 
							"'>More Info</button></td></tr>");

						    $(".modals").append('<div class="modal fade" id="modal' + i + 
						    '"tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title date-title' +
						     i + '"></h4></div><div class="modal-body">' 
						     + '<div class="modal-details">Astronomical twilight Begins: ' 
						     + toTimeZone(data.results.astronomical_twilight_begin) + 
						     '</div>' + '<div class="modal-details">Astronomical Twilight Ends: ' 
						     + toTimeZone(data.results.astronomical_twilight_end) + 
						     '</div><div class="modal-details">Civil Twilight Begins: ' 
						     + toTimeZone(data.results.civil_twilight_begin) + 
						     '</div><div class="modal-details">Civil Twilight Ends: ' 
						     + toTimeZone(data.results.civil_twilight_end) + 
						     '</div>' + '<div class="modal-details">Nautical Twilight Begins: ' 
						     + toTimeZone(data.results.nautical_twilight_begin) + 
						     '</div><div class="modal-details">Nautical Twilight Ends: ' + 
						     toTimeZone(data.results.nautical_twilight_end) + 
						     '</div><div class="modal-details">Solar Noon: ' + 
						     toTimeZone(data.results.solar_noon) + 
							...
							for (var j = 0; j <= dates.length; j++) {
								$(".date" + j).text(dates[j]);
								$(".date-title" + j).text(dates[j]);
							}

							for (var k = 0; k < timesArray.length; k++) {
								timesArray[k].unixDate = moment(dates[k]).unix();
								timesArray[k].date = moment(dates[k]).format("MM-DD-YYYY");
							}
							...
							return data;
								...
								//call this when api call returns
