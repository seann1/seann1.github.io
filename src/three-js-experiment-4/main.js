...
const colorsArray = ["#B1B1B1", "#B1B1B1", "#B1B1B1", "#B1B1B1", "#B1B1B1", "#00796B", "#FFD54F", "#EB437C", "#4DD0E1"];
for (let x = 0; x < 10; x++) {
	for (let y = 0; y < 10; y++) {
		for (let z = 0; z < 10; z++) {
			const cubeGeometry = new THREE.BoxGeometry(1, 1, 1);
			cubeGeometry.scale.x = Math.random();
			const material = new THREE.MeshPhongMaterial({ color: colorsArray[Math.floor(Math.random()*colorsArray.length)] });
...
function animate() {
	...
	if (intersects.length > 0) {
		...
		if (intersects[0].object !== lastIntersected) {
			...
			if (lastIntersected) {
				gsap.to(lastIntersected.scale, { x: Math.random(), y: 0.4, z: 0.4, duration: 10 });
				// gsap.to(lastIntersected.material.color, { r: lastIntersected.originalColor.r, g: lastIntersected.originalColor.g, b: lastIntersected.originalColor.b, duration: 10 });
				
				neighbors.forEach(neighbor => {
					gsap.to(neighbor.scale, { x: Math.random(), y: 0.4, z: 0.4, duration: 10 });
					// gsap.to(neighbor.material.color, { r: neighbor.originalColor.r, g: neighbor.originalColor.g, b: neighbor.originalColor.b, duration: 10 });
				});
				neighbors = [];
			}
